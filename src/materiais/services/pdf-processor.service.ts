...
export class PdfProcessorService {
    ...
    try {
      ...
      for (let i = 0; i < partes.length; i++) {
        ...
        const resumo = await this.glm45Service.gerarTextoEducativo({
          systemPrompt:
            "Você é um especialista em educação. Gere um resumo didático e detalhado do texto abaixo.",
          userPrompt: partes[i],
          maxTokens: 300,
          temperature: 0.7,
          thinking: true,
      ...
      if (resumosParciais.length === 1) {
        ...
        resumoFinal = await this.glm45Service.gerarTextoEducativo({
          systemPrompt:
            "Combine e resuma didaticamente os textos abaixo em um único resumo detalhado.",
      ...
      this.logger.error(
        `Erro ao gerar resumo com IA: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  async chamarAPIHuggingFace(modelo: string, texto: string): Promise<string> {
    try {
      ...
      this.logger.error(
        `Erro ao chamar API HuggingFace: ${error instanceof Error ? error.message : String(error)}`,
      );
      return "";
    }
  }
