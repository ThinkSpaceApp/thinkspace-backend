...
export class MateriaisService {
  ...
  async gerarResumoIaPorTopicos({
    ...
    try {
      ...
      let resumoIA = await this.glm45Service.gerarTextoEducativo({
        systemPrompt: `Você é um educador didático e envolvente. Escreva um texto explicando o conteúdo extraído sobre o tema abaixo de forma clara, acessível e interessante para estudantes do ensino médio.\n\nO texto deve:\n\n- Ter **um título principal** e **subtítulos explicativos** para cada parte.\n- Ser dividido em **5 seções**, conectadas de forma progressiva.\n- Usar **exemplos simples**, analogias quando necessário, e **emojis para ilustrar ideias** (sem exagero, cerca de 1 por parágrafo).\n- **Evitar linguagem rebuscada**, mas sem ser informal demais.\n- Não repetir os tópicos literalmente, mas abordar todos de forma integrada e fluida.\n\nO texto deve estar em português-br e ser formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), e quebras de linha "<br>" para separar parágrafos, além de utilizar marcadores para tópicos dentro de paragrafos, como "-", ou "--" ou "---". Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar.`,
  ...
  async processarPdfEgerarResumo({
    ...
    let prompt = `Você é um educador didático e envolvente. Escreva um texto explicando o conteúdo extraído de um PDF de forma clara, acessível e interessante para estudantes do ensino médio.\n\nO texto deve:\n\n- Ter **um título principal** e **subtítulos explicativos** para cada parte, usando Markdown (# para título principal, ## para subtítulos).\n- Ser dividido em **5 seções**, conectadas de forma progressiva.\n- Usar **exemplos simples**, analogias quando necessário, e **emojis para ilustrar ideias** (cerca de 1 por parágrafo, sem exagero).\n- **Evitar linguagem rebuscada**, mas sem ser informal demais.\n- Não repetir os tópicos literalmente, mas abordar todos de forma integrada e fluida.\n- Não incluir tabelas.\n- Os subtópicos 1. 2. 3. 4. 5. devem ser subtítulos em markdown (##).\n\nO texto deve estar em português-br e ser formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), e quebras de linha (<br>), e tópicos demarcados com (-, -- ou ---) para separar parágrafos. Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar.`;
  ...
  async gerarResumoIaPorMaterial(material: any) {
    ...
    let resumoIA = await this.glm45Service.gerarTextoEducativo({
      systemPrompt: `Você é um educador didático e envolvente. Escreva um texto explicando o conteúdo abaixo de forma clara, acessível e interessante para estudantes do ensino médio.\n\nO texto deve:\n\n- Ter **um título principal** e **subtítulos explicativos** para cada parte.\n- Ser dividido em **5 seções**, conectadas de forma progressiva.\n- Usar **exemplos simples**, analogias quando necessário, e **emojis para ilustrar ideias** (sem exagero, cerca de 1 por parágrafo).\n- **Evitar linguagem rebuscada**, mas sem ser informal demais.\n- Não repetir os tópicos literalmente, mas abordar todos de forma integrada e fluida.\n\nO texto deve estar em português-br e ser formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), e quebras de linha "<br>" para separar parágrafos, além de utilizar marcadores para tópicos dentro de paragrafos, como "-", ou "--" ou "---". Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar.`,
      userPrompt: textoParaResumo,
      maxTokens: 5000,
      temperature: 0.7,
      thinking: false,
