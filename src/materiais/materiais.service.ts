...
export class MateriaisService {
  ...
  async gerarResumoIaPorTopicos({
    ...
    try {
      ...
      let resumoIA = await this.glm45Service.gerarTextoEducativo({
        systemPrompt: `Você é um especialista em educação. Gere um texto extenso, didático e detalhado, explicando o tema e todos os tópicos listados abaixo. O texto deve ser claro, objetivo, acessível para iniciantes e formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), marcadores para cada tópico como (-, -- ou ---) emojis relevantes para cada tópico, e quebras de linha (<br>) para separar parágrafos. Não inclua tabelas. Os subtopicos 1. 2. 3. 4. 5. devem ser subtítulos markdown (##) e não texto normal. Todo o texto deve estar em português-br. Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar. O texto deve abordar diretamente os tópicos, conectando-os de forma natural e progressiva, e pode ser ainda mais longo se necessário para cobrir o assunto de forma completa.`,
  ...
  async criarMaterialComResumoAssunto({
    ...
    try {
      ...
      let resumoIA = await this.glm45Service.gerarTextoEducativo({
        systemPrompt:
          "Você é um especialista em educação. Gere um texto didático e detalhado sobre o tema abaixo. Você é um especialista em educação. Gere um texto extenso, didático e detalhado, explicando o tema e todos os tópicos listados abaixo. O texto deve ser claro, objetivo, acessível para iniciantes e formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), marcadores para cada tópico como (-, -- ou ---) emojis relevantes para cada tópico, e quebras de linha (<br>) para separar parágrafos. Não inclua tabelas. Os subtopicos 1. 2. 3. 4. 5. devem ser subtítulos markdown (##) e não texto normal. Todo o texto deve estar em português-br. Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar. O texto deve abordar diretamente os tópicos, conectando-os de forma natural e progressiva, e pode ser ainda mais longo se necessário para cobrir o assunto de forma completa.",
        userPrompt: textoParaResumo,
        maxTokens: 5000,
        temperature: 0.7,
        thinking: false,
  ...
  async gerarResumoIaPorMaterial(material: any) {
    ...
    let resumoIA = await this.glm45Service.gerarTextoEducativo({
      systemPrompt: `Você é um especialista em educação. Gere um texto extenso, didático e detalhado, dividido em 5 parágrafos, explicando o tema e todos os tópicos listados abaixo. Você é um especialista em educação. Gere um texto extenso, didático e detalhado, explicando o tema e todos os tópicos listados abaixo. O texto deve ser claro, objetivo, acessível para iniciantes e formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), marcadores para cada tópico como (-, -- ou ---) emojis relevantes para cada tópico, e quebras de linha (<br>) para separar parágrafos. Não inclua tabelas. Os subtopicos 1. 2. 3. 4. 5. devem ser subtítulos markdown (##) e não texto normal. Todo o texto deve estar em português-br. Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar. O texto deve abordar diretamente os tópicos, conectando-os de forma natural e progressiva, e pode ser ainda mais longo se necessário para cobrir o assunto de forma completa.`,
      userPrompt: textoParaResumo,
      maxTokens: 5000,
      temperature: 0.7,
      thinking: false,
...
`;
  ...
  const markdownPrompt = `Você é um educador didático e envolvente. Escreva um texto explicando o conteúdo extraído de um PDF de forma clara, acessível e interessante para estudantes do ensino médio.\n\nO texto deve:\n\n- Ter **um título principal** e **subtítulos explicativos** para cada parte.\n- Ser dividido em **5 seções**, conectadas de forma progressiva.\n- Usar **exemplos simples**, analogias quando necessário, e **emojis para ilustrar ideias** (sem exagero, cerca de 1 por parágrafo).\n- **Evitar linguagem rebuscada**, mas sem ser informal demais.\n- Não repetir os tópicos literalmente, mas abordar todos de forma integrada e fluida.\n\nO texto deve estar em português-br e ser formatado em Markdown, usando títulos (#), subtítulos (##), listas, negrito (**termo**), e quebras de linha (<br>), e topicos demoracados com (-, -- ou ---) para separar parágrafos. Não inclua pensamentos, planos, tags como <think> ou estrutura de planejamento. Apenas entregue o texto final, sem introdução sobre o processo de escrita, sem mencionar o que vai fazer ou como vai estruturar.`;
